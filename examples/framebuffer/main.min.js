var sim2dTest = (function (exports) {
  ...
  // Very similar to greggman's module:
  // https://github.com/greggman/webgl-fundamentals/blob/master/webgl/resources/webgl-utils.js
  function createAttributeSetters$1(gl, program) {
    var attribSetters = {};
    var numAttribs = gl.getProgramParameter(program, gl.ACTIVE_ATTRIBUTES);
    for (let i = 0; i < numAttribs; i++) {
      var attribInfo = gl.getActiveAttrib(program, i);
      if (!attribInfo) break;
      var index = gl.getAttribLocation(program, attribInfo.name);
      attribSetters[attribInfo.name] = createAttribSetter$1(gl, index);
    }
    return attribSetters;
  }

  function createAttribSetter$1(gl, index) {
    return function(b) {
      // Enable this attribute (shader attributes are disabled by default)
      gl.enableVertexAttribArray(index);
      // Bind the buffer
  ...
  function setBuffersAndAttributes$1(gl, setters, buffers) {
    setAttributes$1(setters, buffers.attributes);
    if (buffers.indices) {
      gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, buffers.indices.buffer);
    }
  }

  function setAttributes$1(setters, attribs) {
    Object.keys(attribs).forEach( function(name) {
      var setter = setters[name];
      if (setter) setter( attribs[name] );
    });
      ...
      switch (type) {
        ...
        case gl.SAMPLER_CUBE :
          if (isArray) {
            ...
            }(getBindPointForSamplerType$1(gl, type), units);
          } else {
            return function(bindPoint, unit) {
              return function(texture) {
                //gl.uniform1i(location, units); // Typo? How did it even work?
            ...
            }(getBindPointForSamplerType$1(gl, type), textureUnit++);
          }
       default:  // we should never get here
          throw("unknown type: 0x" + type.toString(16));
      }
  ...
  function initShaderProgram$1(gl, vsSource, fsSource) {
    ...
    return {
      program: shaderProgram,
      attributeSetters: createAttributeSetters$1(gl, shaderProgram),
  ...
  function drawOver( gl, programInfo, buffers, uniforms ) {
    ...
    gl.useProgram(programInfo.program);
    setBuffersAndAttributes$1( gl, programInfo.attributeSetters, buffers );
  ...
  function initTexture$1(gl, width, height) {
    // Initializes a 2D texture object, extending the default gl.createTexture()
    // The GL context and the binding target are implicitly saved in the closure.
    // Returns the sampler (as a property) along with update and replace methods.
    // Input data is an ImageData object
    ...
    // Set up mipmapping and anisotropic filtering, if appropriate
    setupMipMaps$1(gl, target, width, height);
    setTextureAnisotropy$1(gl, target);
    ...
    function replace( image ) {
      ...
      // Re-do mipmap setup, since width/height may have changed
      setupMipMaps$1(gl, target, image.width, image.height);
      return;
    }
  ...
  function setupMipMaps$1(gl, target, width, height) {
    // We are using WebGL1 (for compatibility with mobile browsers) which can't
    // handle mipmapping for non-power-of-2 images. Maybe we should provide
    // pre-computed mipmaps? see https://stackoverflow.com/a/21540856/10082269
    if (isPowerOf2$1(width) && isPowerOf2$1(height)) {
      gl.generateMipmap(target);
      // Clamp to avoid wrapping around poles
      // TODO: this may not work with circular coordinates?
      gl.texParameteri(target, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
  ...
  function setTextureAnisotropy$1(gl, target) {
    var ext = (
        gl.getExtension('EXT_texture_filter_anisotropic') ||
        gl.getExtension('MOZ_EXT_texture_filter_anisotropic') || 
        gl.getExtension('WEBKIT_EXT_texture_filter_anisotropic')
  ...
  function initRenderer(gl, fbSize) {
    ...
    const progInfo = initShaderProgram$1(gl, quadShaders.vert, quadShaders.frag);

    // Load data into GPU for shaders: attribute buffers, indices
    const buffers = initQuadBuffers$1(gl);
    // Initialize texture for rendering images to framebuffer. Size is arbitrary?
    const texture = initTexture$1(gl, fbSize.width, fbSize.height);
  ...
  function initRenderer$1(canvas) {
    ...
    // Initialize shader program
    const progInfo = initShaderProgram(gl, shaderSrc.vert, shaderSrc.frag);
    ...
    const ctx2d = canvas2dWrappingFramebuffer(gl, fb);
  ...
  function main() {
    ...
    const map = init(params, renderer.ctx2d, overlay);
